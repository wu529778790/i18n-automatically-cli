...
async function processJavaScriptFile(content, config) {
  ...
  try {
    ...
    traverse(ast, {
      StringLiteral(nodePath) {
        ...
        if (
          ...
          // 替换字符串为 i18n 调用
          const callExpression = parser.parseExpression(
            `${config.scriptI18nCall}('${key}')`
          );
    ...
    if (changes > 0 && config.autoImportI18n && !hasI18nImport) {
      const importStatement = parser.parse(
        `import i18n from '${config.i18nImportPath}';`
      );
...
async function processVueFile(content, config) {
  ...
  try {
    ...
    if (descriptor.template) {
      ...
      // 简单的正则替换中文（这里可以进一步优化）
      const chineseMatches = templateContent.match(
        /[\u4e00-\u9fff\u3400-\u4dbf]+/g
      );
      if (chineseMatches) {
