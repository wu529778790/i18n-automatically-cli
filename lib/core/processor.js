  ...
  try {
    ...
    traverse(ast, {
      ...
      TemplateLiteral(nodePath) {
        ...
        // 这种情况比较复杂，需要重构整个模板字符串
        // 为了保持一致性和避免错误，我们暂时跳过含表达式的模板字符串
        // 这样可以避免计数不一致的问题
      },
    });
    ...
    console.error("处理 JavaScript 文件失败:", error.message);
...
async function processVueFile(content, config, filePath) {
  ...
  try {
    ...
    console.error("处理 Vue 文件失败:", error.message);
    return { content, changes: 0 };
  }
}
  ...
  try {
    ...
    if (fs.existsSync(zhPath)) {
      ...
      try {
        ...
        console.warn(`解析现有语言包失败，创建新的语言包: ${parseError.message}`);
        zhData = {};
      }
    }
    ...
    console.error("更新语言包失败:", error.message);
  }
}
